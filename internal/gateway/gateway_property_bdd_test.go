...
var _ = Describe("Gateway Property Tests", func() {
	// Verifies that regardless of the TenantContext a client supplies
	// in the request, the backend always receives the auth-derived
	// tenant from the identity in the context.
	It("prevents tenant context spoofing", func() {
		rapid.Check(GinkgoT(), func(rt *rapid.T) {
	...
	// Verifies that a non-admin user never sees jobs created by
	// another user in the ListJobs response.
	It("enforces job ownership for non-admin users", func() {
		rapid.Check(GinkgoT(), func(rt *rapid.T) {
	...
	// Verifies that a request without an identity in the context
	// always receives an Unauthenticated error, regardless of which
	// RPC is invoked.
	It("fails closed when no auth identity is present", func() {
		rapid.Check(GinkgoT(), func(rt *rapid.T) {
			svc := gateway.NewService(
				gateway.WithIngestionBackend(&mockIngestion{}),
				gateway.WithCatalogBackend(&mockCatalog{}),
