...
import crosscodexv1connect "github.com/complytime-labs/crosscodex/api/gen/go/crosscodex/v1/crosscodexv1connect"  // Generated Connect handlers/clients
import "connectrpc.com/connect"  // Connect RPC framework
```
...
| **pkg/analyzer**    | `[implemented]` | Generic analyzer plugin interface, type-safe registry, DAG builder with Kahn's algorithm for level-based parallel execution, `Output.ResultData` field carrying each analyzer's JSON-encoded consensus result for durable persistence | pkg/telemetry (optional)          |
...
| **internal/analyzer/requires** | `[implemented]` | Requires analyzer: prerequisite-dependency classification between control pairs (from candidate-generated pairs) via multi-sample LLM panel voting, consensus aggregation via `pkg/analyzer/consensus.Computer` into JSON-encoded `Output.ResultData`, OTel tracing and metrics; persisting `ResultData` and publishing NATS events remain pipeline-layer concerns handled by the orchestrator after `Aggregate` returns | pkg/analyzer, pkg/analyzer/consensus, pkg/analyzer/results, pkg/llmclient, pkg/prompt, pkg/config, pkg/tenant, pkg/telemetry |
| **internal/analysis** | `[implemented]` | Analysis Engine: DAG-based analyzer orchestration, NATS work dispatch, result collection with retry, fail-closed persistence (`ReportStageCompleted` failures fail the analyzer rather than proceeding uncommitted), two-pass resume support via `ExecutionRequest.CompletedOutputs` pre-seeding | pkg/analyzer, pkg/natsbus, pkg/config, pkg/tenant, pkg/telemetry |
...
| **internal/graph** | `[implemented]` | Graph Service: Connect read/write RPCs, NATS subscriber for event-driven materialization, tenant-aware `PGResolver` (live — reads through `db.TenantConnection`, no longer dormant), proto conversion, tenant-scoped queries and traversals | pkg/graphdb, pkg/vectordb, pkg/natsbus, pkg/authn, pkg/tenant, pkg/telemetry |
| **internal/pipeline** | `[implemented]` | Pipeline Service: job lifecycle orchestration, two-pass DAG-based stage execution (pre-candidate classify/embedding/artifacts pass, a `candidate_generation` stage, then a candidate-dependent requires/relationship pass), `CandidateGenerator` persisting `requires_candidates`/`relationship_candidates`, `CompleteAnalysisStage` for atomic `analysis_results` persistence, retry with reset-from-failure, in-toto attestation chain, NATS state events, OTel tracing and metrics | pkg/analyzer, pkg/attestation, pkg/natsbus, pkg/storage, pkg/config, pkg/tenant, pkg/telemetry, internal/analysis, internal/synthesis |
| **cmd/crosscodex**  | `[implemented]` | Cobra-based CLI binary with Connect daemon connectivity, embedded single-node mode, JSON/plain output, shell completion | pkg/config, pkg/prompt, pkg/attestation, internal/gateway, internal/version |
...
- **All internal/ services** — must call `telemetry.Init()` at startup and propagate trace context via NATS message headers and Connect request headers
...
**buf.gen.yaml** - Protobuf code generation (Go + Connect)
...
- **internal/graph implementation** — Graph Service implementing Connect read/write RPCs (GetNode, GetEdge, Traverse, Query, SimilaritySearch, TemporalQuery, CreateNode, CreateEdge, BulkCreateEdges, SupersedeFact) with CQRS architecture: event-driven writes via NATS subscriber consume pipeline stage completion events and materialize nodes/edges into per-tenant Apache AGE graphs, synchronous reads via gRPC. ResourceResolver abstraction with PGResolver for fetching analysis results. Proto conversion layer. OTel tracing and metrics on every handler. Comprehensive test suite: BDD unit tests, property-based tests with pgregory.net/rapid, integration tests with tenant isolation enforcement.
- **internal/pipeline implementation** — Pipeline Service: job lifecycle orchestration with DAG-based stage execution via analysis engine, synthesis executor, and async graph materialization. Connect RPCs (CreateJob, GetJob, ListJobs, CancelJob, RetryJob, Start, Stop) with tenant isolation via RLS-scoped PGStore. Retry supports reset-from-failure (partial re-execution from first failed stage) and full re-creation. In-toto attestation chain: layout creation from DAG, per-stage link generation with material/product digests, chain verification, bundle storage. NATS event publishing for job state and stage transitions. OTel tracing and metrics (job counter, duration histogram). DBStageReporter bridges analysis engine stage events to both database persistence and NATS publishing. Graceful shutdown via Stop() with cancel-all and WaitGroup drain with timeout.
- **cmd/crosscodex** — Cobra-based CLI binary implementing full command tree (project, config, catalog, run, results, prompt, version, completion) with Connect daemon connectivity, embedded single-node auto-start, guided argument errors, JSON/plain/colored output modes, XDG-compliant configuration, and BDD test suite.
- **cmd/crosscodex config get** — Dot-notation config value lookup subcommand (`config get <key>`) with scalar direct output and YAML subtree rendering, `--json` support, and guided argument errors.
- **Connect migration** — Migrated gateway, CLI, and all internal services from gRPC + grpc-gateway to ConnectRPC (connectrpc.com/connect). Consolidated dual-listener server (gRPC + HTTP) to single Connect HTTP server. Added StreamDocument client-streaming RPC for chunked uploads. ServerConfig simplified from GRPCAddr/HTTPAddr to single Addr. New CLISettings.UploadChunkSize config field. Proto codegen migrated from buf.build/grpc/go to buf.build/connectrpc/go.
- **Durable analysis pipeline** — Added `analysis_results` (JSONB blob spine, RLS-scoped) and `relationship_candidates` relational tables (migration 002). All five analyzers (classify, embedding, artifacts, relationship, requires) now encode their final result set into `Output.ResultData`, persisted atomically with stage completion via `Store.CompleteAnalysisStage`. `internal/analysis/engine.go`'s `executeAnalyzer` now fails an analyzer outright if persisting its result fails (previously fire-and-forget), and supports two-pass execution via `ExecutionRequest.CompletedOutputs` pre-seeding. New `internal/pipeline.CandidateGenerator` (with new `CandidateConfig.MinEmbeddingCoverage` fail-closed threshold) generates and persists `requires_candidates`/`relationship_candidates` between the pre-candidate and candidate-dependent analyzer passes. New `cmd/crosscodexd` daemon bootstrap wires the tenant-aware `PGResolver` and `graph.Service`'s NATS subscriber into a real running process for the first time. A new `Store.GetCompletedAnalysisResults` method lets a resumed job (after a daemon crash or restart) reload analyzer outputs already durably committed by a prior process invocation and pre-seed them into the engine's dependency gating, so already-completed analyzers are not redundantly re-executed across a process restart — distinct from `ExecutionRequest.CompletedOutputs`, which only threads state within a single in-process `Execute()` call.
