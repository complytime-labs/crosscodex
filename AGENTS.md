...
| **cmd/crosscodex**  | `[implemented]` | Cobra-based CLI binary with gRPC daemon connectivity, embedded single-node mode, JSON/plain output, shell completion | pkg/config, pkg/prompt, pkg/attestation, internal/gateway, internal/version |
...
- **cmd/crosscodex** — Cobra-based CLI binary implementing full command tree (project, config, catalog, run, results, prompt, version, completion) with gRPC daemon connectivity, embedded single-node auto-start, guided argument errors, JSON/plain/colored output modes, XDG-compliant configuration, and BDD test suite.
